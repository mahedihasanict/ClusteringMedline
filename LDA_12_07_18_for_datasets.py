...
def model_training_and_testing_also_main_function():
    ...
    for key,value in return_dictionary.items():
        ...
        for iii in testingCorpus:
            dist_of_docs_over_topics.append(ldaModel[iii])
        ...
        distanceFromJSD(dataset_name)
        print 'Measuring distance completed'
        
        similarityFromJSD(dataset_name)
...
def spectralClustering(number_of_topics_in_a_dataset,dataset_name): 
    f7 = open('F:\\publication work\\Data\\temp\\14.document_similarity_from_jsd'+dataset_name+'.txt','r')
    df=json.load(f7)
...
def similarityFromJSD(dataset_name):
    similarity_list_from_jsd=list()
    similarity_list_from_jsd_indexer=-1
    f24 = open('F:\\publication work\\Data\\temp\\13.document_distance_from_jsd'+dataset_name+'.txt','r')
    df=json.load(f24)
    
    for ij in df:
        similarity_list_from_jsd.append([])
        similarity_list_from_jsd_indexer+=1
        for jk in ij:
            similarity_list_from_jsd[similarity_list_from_jsd_indexer].append(1-jk) 
    #print df[0][199]
    
    with open('F:\\publication work\\Data\\temp\\14.document_similarity_from_jsd'+dataset_name+'.txt','w') as f25:
        f25.write(str(similarity_list_from_jsd))
    del df
    f24.close
...
def distanceFromJSD(dataset_name):
    jsdlist=list()
    jsdlistindexer=-1
    f28 = open('F:\\publication work\\Data\\temp\\12.distribution_of_topics_in_docs_bracket_replaced_only_prob'+dataset_name+'.txt','r')
    df=json.load(f28)
    #print (df[0])
    #print (df[1])
    #print len(df)
    ...
    for ij in df:
        ...
        for jk in df:
            if ij==jk:
                jsdlist[jsdlistindexer].append(0.0)
                continue
            q=jk
            pq_indexer=-1
            for i in zip(p,q):
                pq_indexer=pq_indexer+1
                if i==(0,0):
                    del p[pq_indexer]
                    del q[pq_indexer]
                    pq_indexer=pq_indexer-1
            if not p or not q:
                jsdlist[jsdlistindexer].append(1.0)
            else:
                jsdlist[jsdlistindexer].append(jsd.JSD_core(p,q))
            
    normalisedJsdList=list()
    maxValue=0.0
    for i in jsdlist:
        if max(i)>maxValue:
            maxValue=max(i)
    if maxValue==0.0:
        maxValue=1.0
    normalisedJsdListindexer=-1
    for j in jsdlist:
        normalisedJsdList.append([])
        normalisedJsdListindexer+=1
        for value in j:
            normalisedJsdList[normalisedJsdListindexer].append(value/maxValue)
    with open('F:\\publication work\\Data\\temp\\13.document_distance_from_jsd'+dataset_name+'.txt','w') as f29:
        f29.write(str(normalisedJsdList))
